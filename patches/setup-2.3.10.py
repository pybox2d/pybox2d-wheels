...
library_base = (
    "library"  # the directory where the egg base will be for setuptools develop command
)
...
def write_init():
    ...
    init_source = [
        ...
        "__version_info__ = (%s,%d)"
        % (box2d_version.replace(".", ","), release_number),
...
box2d_source_files = [os.path.join(source_dir, swig_source)] + sum(
    [glob(os.path.join(path, "*.cpp")) for path in source_paths], []
)
...
pybox2d_extension = Extension(
    "Box2D._Box2D", box2d_source_files, extra_compile_args=extra_args, language="c++"
)

LONG_DESCRIPTION = """ 2D physics library Box2D %s for usage in Python.
...
setup_dict = dict(
    ...
    test_suite="tests",
    options={
        "build_ext": {"swig_opts": swig_arguments},
